...
class ProductListViewCandy extends StatelessWidget {
  final int? items;
  final String? category;
  const ProductListViewCandy
  ({
    super.key,
    this.items,
    this.category,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
        children: [
          Consumer<ProductCandyProvider>(builder: (context, productCategoryProvider, child) => productCategoryProvider.isloading ? const Center(child: CircularProgressIndicator())
          :Expanded(
