import 'package:bee_haak_app/Providers/product_nature_provider.dart';
import 'package:bee_haak_app/dtos/responses/product_responses_dto.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class ProductListViewNature extends StatelessWidget {
  final int? items;
  final String? category;
  const ProductListViewNature
  ({
    super.key,
    this.items,
    this.category,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
        children: [
          Consumer<ProductNatureProvider>(builder: (context, productCategoryProvider, child) => productCategoryProvider.isloading ? const Center(child: CircularProgressIndicator())
          :Expanded(
